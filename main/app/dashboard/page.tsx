...
import { ChevronLeft, ChevronRight, Search, Plus, Menu, Edit, Calendar, Maximize2 } from 'lucide-react';
    ...
    const handleUpdateProject = async () => {
        try {
            ...
            const response = await fetch('/api/updateProject', {
                method: 'POST',
                headers: headers,
                body: JSON.stringify({ status: selectedProject?.status, id: selectedProject?._id }),
            });
    ...
    return (
        <div className="flex flex-col min-h-screen bg-background text-foreground">
            ...
            <Dialog open={isDialogOpen} onOpenChange={handleCloseDialog}>
                <DialogContent className={`bg-background text-foreground ${isExtendedView ? 'sm:max-w-[90vw]' : 'sm:max-w-[425px]'}`}>
                    <DialogHeader>
                    ...
                    <div className={`flex ${isExtendedView ? 'flex-row' : 'flex-col'}`}>
                        <div className={`${isExtendedView ? 'w-1/2 pr-4' : 'w-full'}`}>
                            {!selectedProject || isEditing ? (
                                    ...
                                    <div className="grid gap-4 py-4">
                                        ...
                                        <div className="grid grid-cols-4 items-center gap-4">
                                            ...
                                            <Select
                                                ...
                                                onValueChange={(value) => selectedProject ? setSelectedProject({ ...selectedProject, status: value as 'not-started' | 'working' | 'finished' }) : setNewProject((prev) => ({ ...prev, status: value as 'not-started' | 'working' | 'finished' }))}
                                            >
                                ...
                                <>
                                    <div className="grid gap-4 py-4">
                                        ...
                                        <p><strong>Date:</strong> {format(new Date(selectedProject.date), 'MMMM d, yyyy')}</p>
                        ...
                        {(isExtendedView || (selectedProject && selectedProject.location)) && (
                            <div className={`${isExtendedView ? 'w-1/2 pl-4' : 'w-full mt-4'}`}>
                                <MapComponent
                                    height="100%"
                                    width="100%"
                                    theme="light"
                                    onLocationChange={(coords) => console.log(coords)}
                                />;
                            </div>
                        )}
                    </div>
